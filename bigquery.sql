...
LAST_VALUE(max(c.Battery_Level____))
    OVER (ORDER BY max(c.Battery_Level____) ASC) as usable_battery_level,
'' as motorized_charge_port,
'' as charge_limit_soc_std,
'' as not_enough_power_to_heat,
max(c.Battery_Level____) as battery_level,
battery_level - usable_battery_level AS charge_energy_added,
