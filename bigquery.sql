SELECT 
RANK() OVER (ORDER BY TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(b.Timestamp__UTC_), 5*60)) ASC) AS data_id,
TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(b.Timestamp__UTC_), 5*60)) as Date,
'' as calendar_enabled,
...
LEFT JOIN `MY_PROJECT.MY_VIN.vehicle_states` v ON TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(v.Timestamp__UTC_), 5*60))= TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(b.Timestamp__UTC_), 5*60))
LEFT JOIN `MY_PROJECT.MY_VIN.climate_states` clim ON TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(clim.Timestamp__UTC_), 5*60))= TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(b.Timestamp__UTC_), 5*60))
LEFT JOIN `MY_PROJECT.MY_VIN.driving_states` d ON TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(d.Timestamp__UTC_), 5*60))= TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(b.Timestamp__UTC_), 5*60))
LEFT JOIN `MY_PROJECT.MY_VIN.charging_states` c ON TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(b.Timestamp__UTC_), 5*60))= TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(c.Timestamp__UTC_), 5*60))
WHERE FORMAT_TIMESTAMP('%Y-%m', b.Timestamp__UTC_) = 'MY_MONTH'
GROUP BY b.Timestamp__UTC_, TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(b.Timestamp__UTC_), 5*60))
order by TIMESTAMP_SECONDS(5*60 * DIV(UNIX_SECONDS(b.Timestamp__UTC_), 5*60))