SELECT 
RANK() OVER (ORDER BY FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', clim.Timestamp__UTC_) ASC) AS data_id,
FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', clim.Timestamp__UTC_) as Date,
'' as calendar_enabled,
...
'' as battery_heater_on,
'' as managed_charging_start_time,
max(c.Battery_Level____) as battery_range,
max(c.Charger_Power__kW_) as charger_power,
'' as charge_limit_soc,
'' as charger_pilot_current,
'' as charge_port_latch,
'' as battery_current,
'' as charger_actual_current,
'' as scheduled_charging_pending,
'' as fast_charger_type,
max(c.Usable_Battery_Level____) as usable_battery_level,
...
max(c.Battery_Level____) - LAST_VALUE(max(c.Battery_Level____))
    OVER (ORDER BY max(c.Battery_Level____) ASC) AS charge_energy_added,
'' as charge_port_door_open,
...
max(clim.Inside_Temp____C_) as inside_tempF,
'' as driver_temp_settingF,
max(clim.Outside_Temp____C_) as outside_tempF,
'' as odometerF,
0 as idleNumber,
0 as sleepNumber,
0 as driveNumber,
...
'' as elevation
FROM `MY_PROJECT.MY_VIN.climate_states` clim
LEFT JOIN `MY_PROJECT.MY_VIN.vehicle_states` v ON FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', clim.Timestamp__UTC_)= FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', v.Timestamp__UTC_)
LEFT JOIN `MY_PROJECT.MY_VIN.charging_states` c ON FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', c.Timestamp__UTC_)= FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', clim.Timestamp__UTC_)
LEFT JOIN `MY_PROJECT.MY_VIN.driving_states` d ON FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', d.Timestamp__UTC_)= FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', clim.Timestamp__UTC_)
LEFT JOIN `MY_PROJECT.MY_VIN.battery_states` b ON FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', b.Timestamp__UTC_)= FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', clim.Timestamp__UTC_)
GROUP BY clim.Timestamp__UTC_, FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', clim.Timestamp__UTC_)
order by FORMAT_TIMESTAMP('%Y-%m-%d %H:%M:00', clim.Timestamp__UTC_)